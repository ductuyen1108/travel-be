 ...
 * Validate hex color
 */
type ValidationHexColorOptions = { required?: boolean } & I18nMessage;
export function IsValidHexColor(
  { required = true, message }: ValidationHexColorOptions = { required: true },
): PropertyDecorator {
  return function (target: any, propertyKey: string): void {
    message =
      message || ([`common.word.${propertyKey}`, 'common.word.invalid'] as any);

    IsHexColor({ message: JSON.stringify(message) })(target, propertyKey);
    if (required) {
      IsNotEmpty({ message: JSON.stringify(message) })(target, propertyKey);
    } else
      IsOptional({ message: JSON.stringify(message) })(target, propertyKey);
